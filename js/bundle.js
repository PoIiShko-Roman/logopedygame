/*
  Файл: js/bundle.js
  Зібрано автоматично з окремих скриптів.
  Порядок дотримує залежності, дублікати видалені.
*/

/* ---- BEGIN js/interact.min.js ---- */
...
(function(){
  ...
  function getRandomElements(arr, n){
    ...
    if (n >= arr.length) return shuffleArray(arr).slice(0, arr.length);
    const a = shuffleArray(arr);
    return a.slice(0, n);
  }
  window.UtilsRandom = { shuffleArray, getRandomElements };
})();

(function(){
  function debounce(fn, wait){
    var t = null; var ctx, args;
    function later(){ t = null; fn.apply(ctx, args); ctx = args = null; }
    return function(){ ctx = this; args = arguments; if (t) { clearTimeout(t); } t = setTimeout(later, wait||100); };
  }
  function throttle(fn, wait){
    var last = 0; var t = null; var pendingArgs = null; var ctx;
    function run(){ last = Date.now(); t = null; if (pendingArgs){ fn.apply(ctx, pendingArgs); pendingArgs = null; } }
    return function(){ ctx = this; var now = Date.now(); var rem = wait - (now - last); if (rem <= 0){ last = now; fn.apply(ctx, arguments); } else { pendingArgs = arguments; if (!t) t = setTimeout(run, rem); } };
  }
  window.TimingUtils = { debounce, throttle };
})();

(function(){
  function escapeXml(unsafe){
    return unsafe.replace(/[&<>"]+/g, function(ch){
      switch(ch){case '&': return '&amp;'; case '<': return '&lt;'; case '>': return '&gt;'; case '"': return '&quot;'; default: return ch;}
    });
  }
  function svgPlaceholder(text, w, h){
    var width = w || 100; var height = h || 80;
    var label = (text || 'Зображення').toString().slice(0, 12);
    var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="'+width+'" height="'+height+'" viewBox="0 0 '+width+' '+height+'">'
      + '<rect width="100%" height="100%" fill="#DDD"/>'
      + '<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#555" font-size="14" font-family="sans-serif">'
      + escapeXml(label)
      + '</text>'
      + '</svg>';
    return 'data:image/svg+xml;utf8,' + encodeURIComponent(svg);
  }
  function withImageFallback(img, text){
    if(!img) return;
    function onerr(){ img.removeEventListener('error', onerr); img.src = svgPlaceholder(text, img.width||100, img.height||80); }
    img.addEventListener('error', onerr);
  }
  window.ImagesUtil = { withImageFallback, svgPlaceholder };
})();
(function(){
  function _applyFallbacks(root){
    try{
      if(!window.ImagesUtil) return;
      var scope = root || document;
      var imgs = scope.querySelectorAll('img');
      imgs.forEach(function(img){
        try{
          if(!img.dataset.fallbackApplied){
            ImagesUtil.withImageFallback(img, img.getAttribute('alt') || 'Зображення');
            img.dataset.fallbackApplied = '1';
          }
          if (img.complete && img.naturalWidth === 0) {
            var w = img.width || 200; var h = img.height || 160;
            img.src = ImagesUtil.svgPlaceholder(img.getAttribute('alt') || 'Зображення', w, h);
          }
        }catch(e){}
      });
    }catch(e){}
  ...
  // DOM ready
  try{ document.addEventListener('DOMContentLoaded', function(){ _applyFallbacks(document); }); }catch(e){}
  // Route/page change
  try{ window.addEventListener('hashchange', function(){ setTimeout(function(){ var active = document.querySelector('.page-section.active') || document; _applyFallbacks(active); }, 0); }); }catch(e){}
})();
...
/*
  ФАЙЛ: js/data.js
  ПРИЗНАЧЕННЯ: Локалізація, таксономія гри (G1.*), менеджер контенту слів і дані для головної.
  Використовується на всіх сторінках. Кодування: UTF‑8.
*/

// --- Локалізація (українська) ---
(function(){
  var E = {
    interactMissing: "Помилка: Не вдалося завантажити Interact.js",
    paramsMissing: "Помилка: Не вдалося отримати параметри гри.",
    unknownType: "Помилка: Невідомий тип гри \"{type}\".",
    missingLevelData: "Помилка: База даних слів для \"{level}\" не знайдена.",
    notEnoughWords: "На жаль, для рівня \"{type} + {level}\" недостатньо слів.",
    questionsBuildError: "Помилка формування питань. Спробуй натиснути \"Далі\" ще раз."
  };
  var U = {
    common: { correct: "Правильно!", incorrect: "Неправильно, спробуй ще раз!" },
    questions: { header: "Питання {current}/{total}:" },
    recall: {
      header: "Пригадай 6 вагонів:",
      exceedSelection: "Вже вибрано {count}. Натисни \"Готово\".",
      feedback: { success: "Вітаємо! Все правильно!", fail: "Не зовсім... Правильних: {correct}/{total}. Натисни \"Грати ще раз\"." },
      done: "Гра завершена!",
      memoryError: "Помилка у пам'яті"
    }
  };
  function t(path, vars){
    try {
      var root = { errors: E, ui: U };
      var parts = (path||'').split('.');
      var res = root; for (var i=0;i<parts.length;i++){ if (res && typeof res==='object' && parts[i] in res) res = res[parts[i]]; else return ''; }
      if (typeof res === 'function') return res(vars||{});
      if (typeof res === 'string'){
        if (vars) return res.replace(/\{(\w+)\}/g, function(_,k){ return (vars[k]!==undefined? (''+vars[k]) : ''); });
        return res;
      }
      return '';
    } catch(e){ return ''; }
  }
  window.I18N_UA = { errors: E, ui: U, t: t };
  window.I18N = { t: t };
})();

// --- Константи/таксономія гри ---
(function(){
  const LETTERS = ['с','з','ц','ш','ж','ч','щ','л','р'];
  const CATEGORIES = ['овочі','фрукти','одяг','тварина','птах','посуд'];
  const POSITIONS = ['на початку','в середині','в кінці'];
  const TYPE_TAG_MAP = {
    'овочі': 'овочі', 'фрукти': 'фрукти', 'одяг': 'одяг', 'тварини': 'тварина', 'птахи': 'птах', 'посуд': 'посуд',
    'на початку слова': 'на початку', 'в середині слова': 'в середині', 'в кінці слова': 'в кінці'
  };
  window.G1 = window.G1 || {};
  window.G1.LETTERS = LETTERS;
  window.G1.CATEGORIES = CATEGORIES;
  window.G1.POSITIONS = POSITIONS;
  window.G1.TYPE_TAG_MAP = TYPE_TAG_MAP;
  window.G1.USE_IMAGES = false; // true — показувати реальні зображення
  window.G1_DEBUG = false; // true — детальні валідаційні логи валідації
})();

// --- Менеджер контенту слів по літерах ---
(function(){
  var baseContent = {};   // резерв на майбутнє
  var perLetter = {};     // дані, підвантажені по літері

  function validateLetter(letter, arr){
    try {
      if (!Array.isArray(arr)) return;
      var seen = new Set();
      var cats = (window.G1 && window.G1.CATEGORIES) || [];
      var poss = (window.G1 && window.G1.POSITIONS) || [];
      var DBG = !!(window.G1_DEBUG);
      var CL = function(msg){ try{ if(DBG) console.debug('[G1 validate]', msg); }catch(_){} };
      arr.forEach(function(it){
        if (!it || typeof it.word !== 'string') { CL("letter '"+letter+"': item without word"); return; }
        var w = it.word.trim(); if (seen.has(w)) CL("letter '"+letter+"': duplicate word '"+w+"'"); else seen.add(w);
        var tags = Array.isArray(it.tags)? it.tags: [];
        var cat = tags.find(function(t){ return cats.indexOf(t) >= 0; });
        var pos = tags.find(function(t){ return poss.indexOf(t) >= 0; });
        if (!pos) CL("word '"+w+"': position tag missing or unknown");
        if (!cat && !pos) CL("word '"+w+"': category tag missing or unknown");
        if (tags.indexOf(letter) < 0) CL("word '"+w+"': letter tag '"+letter+"' missing in tags");
      });
    } catch(_){}
  }

  function mergeContent(){
    var merged = Object.assign({}, baseContent);
    try { Object.keys(perLetter).forEach(function(k){ var v = perLetter[k]; if (Array.isArray(v) && v.length > 0) merged[k] = v; }); } catch(_){}
    window.G1 = window.G1 || {}; window.G1.CONTENT = merged;
    try { Object.keys(merged).forEach(function(k){ validateLetter(k, merged[k]); }); } catch(_){}
  }

  // Початкове підхоплення глобальних масивів, якщо підключені раніше
  try {
    if (Array.isArray(window.G1_S) && window.G1_S.length) perLetter['с'] = window.G1_S;
    if (Array.isArray(window.G1_SH) && window.G1_SH.length) perLetter['ш'] = window.G1_SH;
    if (Array.isArray(window.G1_R) && window.G1_R.length) perLetter['р'] = window.G1_R;
    if (Array.isArray(window.G1_L) && window.G1_L.length) perLetter['л'] = window.G1_L;
    if (Array.isArray(window.G1_Z) && window.G1_Z.length) perLetter['з'] = window.G1_Z;
    if (Array.isArray(window.G1_TS) && window.G1_TS.length) perLetter['ц'] = window.G1_TS;
    if (Array.isArray(window.G1_ZH) && window.G1_ZH.length) perLetter['ж'] = window.G1_ZH;
    if (Array.isArray(window.G1_CH) && window.G1_CH.length) perLetter['ч'] = window.G1_CH;
    if (Array.isArray(window.G1_SHCH) && window.G1_SHCH.length) perLetter['щ'] = window.G1_SHCH;
  } catch(_) {}
  mergeContent();

  // Публічні API
  function getByLetter(letter){ var m = (window.G1 && window.G1.CONTENT) || {}; return m[letter] || []; }
  function findBy(letter, predicate){ return getByLetter(letter).filter(predicate); }
  function validate(letter, tag, reqCorrect, reqIncorrect){ var list = getByLetter(letter); var correct = list.filter(function(w){return (w.tags||[]).includes(tag);}); var incorrect = list.filter(function(w){return !(w.tags||[]).includes(tag);}); return { ok: correct.length >= reqCorrect && incorrect.length >= reqIncorrect, correctCount: correct.length, incorrectCount: incorrect.length }; }

  // Ліниве підвантаження файлу конкретної літери з js/letters/<slug>.js
  var LETTER_FILE_MAP = { 'с': 's', 'з': 'z', 'ц': 'ts', 'ш': 'sh', 'ж': 'zh', 'ч': 'ch', 'щ': 'shch', 'л': 'l', 'р': 'r' };
  function ensureLetterLoaded(letter){
    return new Promise(function(resolve){
      if (!letter) return resolve();
      var exists = getByLetter(letter); if (Array.isArray(exists) && exists.length) return resolve();
      var slug = LETTER_FILE_MAP[letter]; if (!slug) return resolve();
      var already = document.querySelector('script[data-g1-letter="'+letter+'"]'); if (already) return resolve();
      var s = document.createElement('script');
      s.src = 'js/letters/'+slug+'.js'; s.defer = true; s.async = true; s.setAttribute('data-g1-letter', letter);
      s.onload = function(){ try { var name = 'G1_'+slug.toUpperCase(); var arr = window[name]; if (Array.isArray(arr) && arr.length) { perLetter[letter] = arr; } } catch(_){} mergeContent(); resolve(); };
  ...
  window.G1 = window.G1 || {};
  window.G1.getByLetter = getByLetter;
  window.G1.findBy = findBy;
  window.G1.validate = validate;
  window.G1.ensureLetterLoaded = ensureLetterLoaded;
})();

// --- Дані для головної сторінки (вміст картки «Гра Потяг») ---
const GAME_DATA = {
  "1": {
    title: "Гра Потяг",
    subtitle: "Збирай потяг, вимовляй слова, запам’ятовуй порядок і відповідай на запитання.",
    optionsGroups: [
      { text: "Овочі", targetPage: "index.html" },
      { text: "Фрукти", targetPage: "index.html" },
      { text: "Одяг", targetPage: "index.html" },
      { text: "Тварини", targetPage: "index.html" },
      { text: "Птахи", targetPage: "index.html" },
      { text: "Посуд", targetPage: "index.html" },
      { text: "На початку слова", targetPage: "index.html" },
      { text: "В середині слова", targetPage: "index.html" },
      { text: "В кінці слова", targetPage: "index.html" }
    ]
  }
};
...
/* ---- BEGIN js/train_game.js ---- */
(function(){
  function applyState(ctx, state){
    if (!ctx || !ctx.gameContainer) { if (ctx) ctx.gameState = state; return; }
    ctx.gameState = state;
    var c = ctx.gameContainer;
    c.classList.remove('is-assembling','is-quiz','is-recall','is-finished');
    switch(state){
      case 'ASSEMBLING': c.classList.add('is-assembling'); break;
      case 'QUESTIONS': c.classList.add('is-quiz'); break;
      case 'RECALL': c.classList.add('is-recall'); break;
      case 'FINISHED': c.classList.add('is-finished'); break;
    }
  }
  window.Game1Engine = { applyState };
})();

(function(){
  function updateGameStatus(ctx, s, m){
  if (ctx.indicator) {
    ctx.indicator.className = 'game-indicator ' + s;
    ctx.indicator.title = ({ grey: 'Завантаження...', red: 'Помилка!', green: 'Готово!' })[s] || '';
  }
  if (ctx.gameMessage) {
    var l = (ctx.currentTaskLevel || '?').toUpperCase();
    var t = ctx.currentTaskType || 'Рівень';
    var cm = '';
    switch(ctx.gameState){
      case 'ASSEMBLING':
        cm = s === 'grey' ? ('Рівень: [' + l + ']. Тип "' + t + '". Розстав 6 правильних вагонів.')
           : s === 'red' ? 'Ще не все правильно. Спробуй ще.'
           : ('Готово! Тип "' + t + '" [' + l + ']. Натисни "Далі".');
        break;
      case 'QUESTIONS':
        cm = s === 'green' ? 'Правильно!'
           : s === 'red' ? 'Неправильно, спробуй ще!'
           : ('Питання ' + (ctx.currentQuestionIndex + 1) + '/' + ((ctx.generatedQuestions||[]).length || 0) + ':');
        break;
      case 'RECALL':
        cm = m ? m : ('Пригадай 6 вагонів "' + t + '" [' + l + ']. (усього ' + (ctx.TOTAL_SLOTS || 6) + ')');
        break;
      case 'FINISHED':
        cm = s === 'green' ? 'Гру завершено! Натисни "Грати ще раз".' : 'Спробуй ще раз.';
        break;
      default:
        cm = 'Завантаження...';
    }
    ctx.gameMessage.textContent = m || cm;
    ctx.gameMessage.style.display = 'block';
  }
}
  function showError(ctx, m){ if (ctx.board) ctx.board.style.display = 'none'; updateGameStatus(ctx,'red', m); }
  window.Game1UI = { updateGameStatus: updateGameStatus, showError: showError };
})();

(function(){
  function createSlotElement(ctx, index){
    const s = document.createElement('div'); s.className = 'train-slot'; s.dataset.slotId = index; s.dataset.occupied = 'false'; ctx.trainSlotsContainer.appendChild(s); return s;
  }
  function createWagonElement(ctx, d, i){
    const w = document.createElement('div'); w.className = 'wagon-card'; w.id = `wagon-${i}`; w.dataset.word = d.word;
    w.dataset.isCorrect = ctx.correctWords.includes(d.word).toString(); w.dataset.currentSlot = 'none';
    const img = document.createElement('img'); img.className = 'wagon-image'; img.alt = d.word; img.draggable = false;
try {
  if (window.G1 && window.G1.USE_IMAGES === false) {
    img.src = (window.ImagesUtil ? ImagesUtil.svgPlaceholder(d.word, 200, 160) : '');
  } else {
    if (window.ImagesUtil && typeof ImagesUtil.withImageFallback === 'function') { ImagesUtil.withImageFallback(img, d.word); }
    var _url = 'image/game1/' + d.image;
    img.src = _url;
  }
} catch(_) { img.src = ''; }const label = document.createElement('span'); label.className = 'wagon-label'; label.textContent = d.word; w.appendChild(img); w.appendChild(label);
    ctx.board.appendChild(w); return w;
  }
  function createSlots(ctx, count){ if (!ctx.trainSlotsContainer) return; for(let i=0;i<count;i++){ ctx.slots.push(createSlotElement(ctx,i)); } }
  function createWagons(ctx, list){ if (!ctx.board) return; list.forEach((d,i)=>{ ctx.wagons.push(createWagonElement(ctx,d,i)); }); setTimeout(()=>scatterWagons(ctx),50); }
  function rectsOverlap(a,b,g){ // axis-aligned overlap with gap
    return !(a.x + a.w + g <= b.x || b.x + b.w + g <= a.x || a.y + a.h + g <= b.y || b.y + b.h + g <= a.y);
  }
  function scatterWagons(ctx){
    if (!ctx.spawnArea || !ctx.board) return;
    const sr = ctx.spawnArea.getBoundingClientRect();
    const br = ctx.board.getBoundingClientRect();
    const st = sr.top - br.top; const sl = sr.left - br.left;
    const placed = [];
    const gap = 6; // minimal gap between wagons
    ctx.wagons.forEach(w => {
      if (w.classList.contains('locked')) {
        const sid = w.dataset.currentSlot; if(sid !== 'none' && ctx.slots[sid]) { const s = ctx.slots[sid]; const slr = s.getBoundingClientRect(); const x = slr.left - br.left; const y = slr.top - br.top; w.style.transform = `translate(${x}px, ${y}px)`; w.dataset.x = x; w.dataset.y = y; } return;
      }
      const brw = w.getBoundingClientRect(); const wx = brw.width || ctx.WAGON_WIDTH; const wy = brw.height || ctx.WAGON_HEIGHT;
      const mx = Math.max(0, sr.width - wx - (ctx.SPAWN_PADDING * 2));
      const my = Math.max(0, sr.height - wy - (ctx.SPAWN_PADDING * 2));
      let fx = sl + ctx.SPAWN_PADDING, fy = st + ctx.SPAWN_PADDING;
      let tries = 30; let ok = false;
      while(tries-- > 0){
        const rx = Math.max(0, Math.floor(Math.random() * (mx+1)));
        const ry = Math.max(0, Math.floor(Math.random() * (my+1)));
        fx = sl + ctx.SPAWN_PADDING + rx; fy = st + ctx.SPAWN_PADDING + ry;
        const cand = { x: fx, y: fy, w: wx, h: wy };
        ok = true;
        for (let i=0;i<placed.length;i++){ if (rectsOverlap(cand, placed[i], gap)) { ok = false; break; } }
        if (ok) { placed.push(cand); break; }
      }
      // fallback: even if overlaps after many tries, place last candidate
      w.style.transform = `translate(${fx}px, ${fy}px)`; w.dataset.x = fx; w.dataset.y = fy; w.dataset.spawnX = fx; w.dataset.spawnY = fy;
    });
  }
  window.Game1Board = { createSlotElement, createWagonElement, createSlots, createWagons, scatterWagons };
})();

(function () {
  "use strict";

  // CSS-селектори, що використовуються для ініціалізації DnD
  var SELECTOR = {
    card: ".wagon-card",
    slot: ".train-slot",
    recall: ".recall-wagon",
  ...
  /**
   * Ініціалізує drag & drop для вагончиків і зон дропу.
   * @param {Object} ctx - Контекст гри (Game1_Logic), містить колбеки та DOM-вузли.
   */
  function setup(ctx) {
    if (typeof interact === "undefined") return; // Без interact.js нічого не робимо

    // Перетягування вагончиків
    interact(SELECTOR.card).draggable({
      inertia: true,
      modifiers: [
        interact.modifiers.restrictRect({
          restriction: ctx.board, // обмежуємо рух межами ігрової дошки
          endOnly: false,
        }),
      ],
      autoScroll: true,
      listeners: {
        start: ctx.handleDragStart,
        move: ctx.handleDragMove,
        end: ctx.handleDragEnd,
      },
    });

    // Зони дропу для вагончиків (слоти поїзда)
    interact(SELECTOR.slot).dropzone({
      accept: SELECTOR.card,
      overlap: ctx.DROP_OVERLAP_THRESHOLD,
      listeners: {
        dragenter: ctx.handleDropZoneEnter,
        dragleave: ctx.handleDropZoneLeave,
        drop: ctx.handleDrop,
      },
    });
  }
  ...
  /**
   * Вимикає drag & drop, звільняючи всі ресурси interact.js.
   */
  function unset() {
    try {
      if (typeof interact !== "undefined") {
        interact(SELECTOR.card).unset();
        interact(SELECTOR.slot).unset();
        interact(SELECTOR.recall).unset();
      }
    } catch (_) {}
  }

  // Експортуємо в глобальний простір
  window.Game1DragDrop = { setup: setup, unset: unset };
})();

(function(){
  function unique(arr){ return Array.from(new Set(arr)); }
  function ensureOptions(correct, pool, count){
    const opts = new Set([correct]);
    const candidates = pool.filter(w => w !== correct);
    const shuffled = (window.UtilsRandom? UtilsRandom.shuffleArray(candidates): candidates.sort(()=>Math.random()-0.5));
    for (let i=0; i<shuffled.length && opts.size < count; i++) opts.add(shuffled[i]);
    while (opts.size < count && candidates.length>0) opts.add(candidates[0]);
    return Array.from(opts).slice(0, count);
  }
  function tr(key, vars, fallback){ try{ if(window.I18N && typeof I18N.t==='function'){ const s = I18N.t(key, vars||{}); if(s) return s; } }catch(e){} return fallback||''; }
  function generate(ctx){
    ctx.generatedQuestions = [];
    const wwRaw = ctx.orderedCorrectWagons.map(w => w.dataset.word);
    const ww = unique(wwRaw);
    if (ww.length < ctx.TOTAL_SLOTS) { Game1UI.updateGameStatus(ctx,'red', tr('errors.questionsBuildError',{},'Помилка формування питань. Спробуй натиснути "Далі" ще раз.')); return; }
    const afterQs = [];
    for (let i = 0; i < ctx.TOTAL_SLOTS - 1; i++) {
      const q = `Який вагон після "${ww[i]}"?`;
      const ca = ww[i+1];
      const pool = ww.filter(w => w !== ww[i] && w !== ca);
      const options = ensureOptions(ca, pool, ctx.QUIZ_OPTIONS_COUNT);
      afterQs.push({ q: q, o: shuffle(options), ca: ca });
    }
    const beforeQs = [];
    for (let i = 1; i < ctx.TOTAL_SLOTS; i++) {
      const q = `Який вагон перед "${ww[i]}"?`;
      const ca = ww[i-1];
      const pool = ww.filter(w => w !== ww[i] && w !== ca);
      const options = ensureOptions(ca, pool, ctx.QUIZ_OPTIONS_COUNT);
      beforeQs.push({ q: q, o: shuffle(options), ca: ca });
    }
    const pickN = (arr,n)=> (window.UtilsRandom? UtilsRandom.getRandomElements(arr,n): arr.sort(()=>Math.random()-0.5).slice(0,n));
    const selAfter = pickN(afterQs, Math.min(3, afterQs.length));
    const selBefore = pickN(beforeQs, Math.min(3, beforeQs.length));
    let all = selAfter.concat(selBefore);
    const poolRest = (afterQs.concat(beforeQs)).filter(x => !all.includes(x));
    while (all.length < 6 && poolRest.length){ all.push(poolRest.shift()); }
    ctx.generatedQuestions = shuffle(all).slice(0, 6);
    ctx.currentQuestionIndex = 0;
  }
  function displayCurrent(ctx){
    if (!ctx.questionText || !ctx.answerOptions || ctx.currentQuestionIndex >= ctx.generatedQuestions.length) return;
    ctx.isQuestionActive = true;
    const q = ctx.generatedQuestions[ctx.currentQuestionIndex];
    ctx.questionText.textContent = q.q;
    ctx.answerOptions.innerHTML = '';
    q.o.forEach(ot => { const b = document.createElement('button'); b.className = 'answer-btn'; b.textContent = ot; b.onclick = () => handleAnswerClick(ctx, ot, q.ca, b); ctx.answerOptions.appendChild(b); });
    Game1UI.updateGameStatus(ctx,'grey', tr('ui.questions.header',{current: ctx.currentQuestionIndex + 1, total: ctx.generatedQuestions.length}, 'Питання ' + (ctx.currentQuestionIndex + 1) + '/' + ctx.generatedQuestions.length + ':'));
  }
  function handleAnswerClick(ctx, sa, ca, cb){
    if (!ctx.isQuestionActive) return; ctx.isQuestionActive = false;
    ctx.answerOptions.querySelectorAll('.answer-btn').forEach(b => b.disabled = true);
    if (sa === ca) {
      cb.classList.add('correct'); Game1UI.updateGameStatus(ctx,'green', tr('ui.common.correct',{},'Правильно!'));
      setTimeout(() => { ctx.currentQuestionIndex++; if (ctx.currentQuestionIndex < ctx.generatedQuestions.length) { displayCurrent(ctx); } else { if (ctx.questionsSection) ctx.questionsSection.style.display = 'none'; if (ctx.memoryCheckBtn) ctx.memoryCheckBtn.style.display = 'block'; Game1UI.updateGameStatus(ctx,'green','Чудово! Тепер останнє завдання.'); } }, 900);
    } else {
      cb.classList.add('incorrect'); Game1UI.updateGameStatus(ctx,'red', tr('ui.common.incorrect',{},'Неправильно, спробуй ще!'));
      setTimeout(() => { cb.classList.remove('incorrect'); ctx.answerOptions.querySelectorAll('.answer-btn').forEach(b => b.disabled = false); ctx.isQuestionActive = true; Game1UI.updateGameStatus(ctx,'grey', tr('ui.questions.header',{current: ctx.currentQuestionIndex + 1, total: ctx.generatedQuestions.length}, 'Питання ' + (ctx.currentQuestionIndex + 1) + '/' + ctx.generatedQuestions.length + ':')); }, 1200);
    }
  }
  function pick(arr,n){ return (window.UtilsRandom? UtilsRandom.getRandomElements(arr,n): arr.slice(0,n)); }
  function shuffle(arr){ return (window.UtilsRandom? UtilsRandom.shuffleArray(arr): arr.sort(() => Math.random()-0.5)); }
  window.Game1Questions = { generate, displayCurrent, handleAnswerClick };
})();

(function(){
  function tr(key, vars, fallback){
    try { if (window.I18N && typeof I18N.t === 'function') { const s = I18N.t(key, vars||{}); if (s) return s; } } catch(e){}
    return fallback || '';
  }

  function getRandom(arr, n){ return (window.UtilsRandom ? UtilsRandom.getRandomElements(arr, n) : (arr||[]).slice(0, n)); }
  function shuffle(arr){ return (window.UtilsRandom ? UtilsRandom.shuffleArray(arr) : (arr||[]).sort(() => Math.random() - 0.5)); }

  function setupGrid(ctx){
    if (!ctx || !ctx.recallGrid) return;
    ctx.recallGrid.innerHTML = '';
    const correct = Array.isArray(ctx.correctWords) ? ctx.correctWords.slice(0) : [];
    const incorrect = Array.isArray(ctx.incorrectWordsUsed) ? ctx.incorrectWordsUsed.slice(0) : [];
    const countIncorrect = Math.min(ctx.RECALL_INCORRECT_COUNT || 3, incorrect.length);
    const items = shuffle([ ...correct, ...getRandom(incorrect, countIncorrect) ]);

    items.forEach(word => {
      const base = (ctx.wagons || []).find(w => w.dataset && w.dataset.word === word) || null;
      const el = document.createElement('div');
      el.className = 'wagon-card recall-wagon';
      ...
      if (base) {
        ...
        img.className = 'wagon-image';
        img.alt = word;
        try {
          if (window.ImagesUtil && ImagesUtil.svgPlaceholder) img.src = ImagesUtil.svgPlaceholder(word, 200, 200);
          else { img.src = ''; img.style.background = '#444'; }
        } catch(_) { img.src = ''; img.style.background = '#444'; }
        const label = document.createElement('span');
        label.className = 'wagon-label';
      ...
      el.addEventListener('click', () => handleSelection(ctx, { currentTarget: el }));
      el.addEventListener('keydown', ev => { if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); handleSelection(ctx, { currentTarget: el }); } });
      ctx.recallGrid.appendChild(el);
    });
  }

  function handleSelection(ctx, e){
    if (!ctx || ctx.gameState !== 'RECALL') return;
    const w = e.currentTarget;
    const count = ctx.recallGrid.querySelectorAll('.wagon-card.selected').length;
    if (w.classList.contains('selected')) { w.classList.remove('selected'); w.setAttribute('aria-pressed','false'); }
    else if (count < ctx.TOTAL_SLOTS) { w.classList.add('selected'); w.setAttribute('aria-pressed','true'); }
    else {
      if (ctx.recallFeedback) {
        ctx.recallFeedback.textContent = tr('ui.recall.exceedSelection',{count: ctx.TOTAL_SLOTS}, `Вже вибрано ${ctx.TOTAL_SLOTS}. Натисни 'Готово'.`);
        setTimeout(() => { if (ctx.recallFeedback) ctx.recallFeedback.textContent = ''; }, 2000);
      }
    }
  }

  function submit(ctx){
    if (!ctx || !ctx.recallGrid || ctx.gameState !== 'RECALL') return;
    const selected = Array.from(ctx.recallGrid.querySelectorAll('.wagon-card.selected')).map(w => w.dataset.word);
    let correct = 0; selected.forEach(w => { if ((ctx.correctWords||[]).includes(w)) correct++; });
    const isSuccess = selected.length === ctx.TOTAL_SLOTS && correct === ctx.TOTAL_SLOTS;
    if (ctx.recallFeedback) {
      if (isSuccess) {
        ctx.recallFeedback.textContent = tr('ui.recall.feedback.success',{},'Вітаємо! Все правильно!');
        ctx.recallFeedback.className = 'recall-feedback correct';
        Game1UI.updateGameStatus(ctx,'green', tr('ui.recall.done',{},'Гра завершена!'));
      } else {
        ctx.recallFeedback.textContent = tr('ui.recall.feedback.fail',{correct: correct, total: ctx.TOTAL_SLOTS}, `Не зовсім... Правильних: ${correct}/${ctx.TOTAL_SLOTS}. Натисни "Грати ще раз".`);
        ctx.recallFeedback.className = 'recall-feedback incorrect';
        Game1UI.updateGameStatus(ctx,'red', tr('ui.recall.memoryError',{},'Помилка у пам\'яті'));
      }
    }
    if (isSuccess) {
      Game1Engine.applyState(ctx,'FINISHED');
      if (ctx.recallSubmitBtn) ctx.recallSubmitBtn.style.display = 'none';
      if (ctx.playAgainBtn) ctx.playAgainBtn.style.display = 'block';
    } else {
      if (ctx.recallRetryBtn) ctx.recallRetryBtn.style.display = 'inline-block';
      if (ctx.recallSubmitBtn) ctx.recallSubmitBtn.style.display = 'block';
    }
  }

  function resetSelection(ctx){ if (!ctx || !ctx.recallGrid) return; ctx.recallGrid.querySelectorAll('.wagon-card.selected').forEach(w => w.classList.remove('selected')); if (ctx.recallFeedback) ctx.recallFeedback.textContent=''; }
  function retry(ctx){ if (ctx.recallFeedback) ctx.recallFeedback.textContent=''; if (ctx.recallRetryBtn) ctx.recallRetryBtn.style.display='none'; if (ctx.recallSubmitBtn) ctx.recallSubmitBtn.style.display='block'; setupGrid(ctx); Game1UI.updateGameStatus(ctx,'grey', tr('ui.recall.header',{},'Пригадай 6 вагонів:')); }

  function trapFocus(ctx, enable){
    try {
      if (!ctx || !ctx.recallOverlay) return;
      if (enable) {
        ctx._prevFocus = document.activeElement;
        const focusables = ctx.recallOverlay.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
        if (focusables.length) focusables[0].focus();
        ctx._onKeydownTrap = function(e){
          if (e.key === 'Escape') { try { resetSelection(ctx); if (ctx.recallFeedback) ctx.recallFeedback.textContent = tr('ui.recall.header',{},'Пригадай 6 вагонів:'); } catch(_){} return; }
          if (e.key !== 'Tab') return;
          const list = Array.from(ctx.recallOverlay.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'));
          if (!list.length) return;
          const first = list[0]; const last = list[list.length-1];
          if (e.shiftKey) { if (document.activeElement === first) { e.preventDefault(); last.focus(); } }
          else { if (document.activeElement === last) { e.preventDefault(); first.focus(); } }
        };
        ctx.recallOverlay.addEventListener('keydown', ctx._onKeydownTrap);
      } else {
        if (ctx._onKeydownTrap) { ctx.recallOverlay.removeEventListener('keydown', ctx._onKeydownTrap); }
        if (ctx._prevFocus && ctx._prevFocus.focus) { try { ctx._prevFocus.focus(); } catch(e){} }
        ctx._onKeydownTrap = null; ctx._prevFocus = null;
      }
    } catch(e){}
  }

  window.Game1Recall = { setupGrid, handleSelection, submit, resetSelection, retry, trapFocus };
})();

/*
  ФАЙЛ: js/games/train/orchestrator.js
  ОПИС: Чистий оркестратор для гри 1 (Потяг).
*/
...
class Game1_Logic {
  constructor(params){
    ...
    this.trainArea = this.gameContainer.querySelector('#train-area');
    ...
    this.questionText = this.gameContainer.querySelector('#question-text');
    this.answerOptions = this.gameContainer.querySelector('#answer-options');
    ...
    this.recallResetBtn = this.gameContainer.querySelector('#recall-reset-btn');
    this.recallRetryBtn = this.gameContainer.querySelector('#recall-retry-btn');
    this.recallSubmitBtn = this.gameContainer.querySelector('#recall-submit-btn');
    this.playAgainBtn = this.gameContainer.querySelector('#play-again-btn');

    // 2) Стан
    this.wagons = []; this.slots = []; this.correctWords = []; this.incorrectWordsUsed = []; this.orderedCorrectWagons = [];
    this.currentTaskType = ''; this.currentTaskLevel = ''; this.gameState = 'ASSEMBLING';
    this.generatedQuestions = []; this.currentQuestionIndex = 0; this.isQuestionActive = false;

    // 3) Константи
    this.REQUIRED_CORRECT = 6; this.REQUIRED_INCORRECT = 4; this.TOTAL_SLOTS = 6;
    this.RECALL_INCORRECT_COUNT = 3; this.QUIZ_OPTIONS_COUNT = 3; this.WAGON_WIDTH = 100; this.WAGON_HEIGHT = 100; this.SPAWN_PADDING = 20; this.DROP_OVERLAP_THRESHOLD = 0.5;

    // 4) Прив'язка обробників
    ...
    if (typeof interact === 'undefined') { this.showError('Помилка: Не вдалося завантажити Interact.js'); return; }
    this.initGame(params);
  }

  applyState(state){ if (window.Game1Engine) { Game1Engine.applyState(this, state); } else { this.gameState = state; } }

  resetGameUI(){
    try{
      if (this.board) this.board.querySelectorAll('.wagon-card').forEach(w=>w.remove());
      ...
      if (this.memoryCheckBtn) this.memoryCheckBtn.style.display = 'none';
  ...
  initGame(params){
    ...
    this.currentTaskType = typeTag; this.currentTaskLevel = levelTag;
    if (!typeTag || !levelTag) { this.showError('Помилка: не вказано тип або літеру.'); return; }
    const tagMap = (window.G1 && window.G1.TYPE_TAG_MAP) || { 'овочі':'овочі','фрукти':'фрукти','одяг':'одяг','тварини':'тварина','птахи':'птах','посуд':'посуд','на початку слова':'на початку','в середині слова':'в середині','в кінці слова':'в кінці' };
    const dataTag = tagMap[(typeTag||'').trim().toLowerCase()] || (typeTag||'').trim().toLowerCase();

    // Lazy-load слів для літери
    try{
      const now = (window.G1 && window.G1.CONTENT) || {};
      const needLoad = !Array.isArray(now[levelTag]) || now[levelTag].length === 0;
      if (needLoad && window.G1 && typeof G1.ensureLetterLoaded === 'function'){
        G1.ensureLetterLoaded(levelTag).then(()=>{ try{ this.initGame(params); }catch(_){} });
        return;
      }
    }catch(_){}

    const CONTENT = (window.G1 && window.G1.CONTENT) || {};
    const list = CONTENT[levelTag];
    if (!Array.isArray(list) || list.length === 0){ this.showError('Помилка: Немає слів для обраної літери.'); return; }

    const correctWordsData = list.filter(w => (w.tags||[]).includes(dataTag));
    const incorrectWordsData = list.filter(w => !(w.tags||[]).includes(dataTag));
    if (correctWordsData.length < this.REQUIRED_CORRECT || incorrectWordsData.length < this.REQUIRED_INCORRECT){
      this.showError(`Недостатньо слів для рівня "${typeTag} + ${levelTag.toUpperCase()}".`); return;
    }

    const finalCorrect = this.getRandomElements(correctWordsData, this.REQUIRED_CORRECT);
    const finalIncorrect = this.getRandomElements(incorrectWordsData, this.REQUIRED_INCORRECT);
    this.correctWords = finalCorrect.map(w=>w.word);
    this.incorrectWordsUsed = finalIncorrect.map(w=>w.word);
    const allGameWords = this.shuffleArray(finalCorrect.concat(finalIncorrect));

    if (this.cabinLetterEl) this.cabinLetterEl.textContent = levelTag.toUpperCase();

    if (window.Game1Board){
      Game1Board.createSlots(this, this.TOTAL_SLOTS);
      Game1Board.createWagons(this, allGameWords);
    }

    // DnD + розкидання
    this._debouncedScatter = (window.TimingUtils ? TimingUtils.debounce(()=>this.scatterWagons(),100) : this.scatterWagons);
    window.addEventListener('resize', this._debouncedScatter);
    if (window.Game1DragDrop) Game1DragDrop.setup(this);
    this.scatterWagons();

    // Кнопки етапів
    if (this.nextStageBtn) this.nextStageBtn.addEventListener('click', this.goToQuestionsStage);
    if (this.memoryCheckBtn) this.memoryCheckBtn.addEventListener('click', this.goToRecallStage);
    if (this.recallSubmitBtn) this.recallSubmitBtn.addEventListener('click', this.submitRecall);

    this._onRecallReset = () => { if (window.Game1Recall) Game1Recall.resetSelection(this); };
    this._onRecallRetry = () => { if (window.Game1Recall) Game1Recall.retry(this); };
    if (this.recallResetBtn) this.recallResetBtn.addEventListener('click', this._onRecallReset);
    if (this.recallRetryBtn) this.recallRetryBtn.addEventListener('click', this._onRecallRetry);

    this.applyState('ASSEMBLING');
    this.updateGameStatus('grey');
  }

  // Geometry/helpers
  scatterWagons(){ if (window.Game1Board) Game1Board.scatterWagons(this); }
  handleDragStart(e){ const w = e.target; if (w.classList.contains('locked')) { this.unlockWagon(w, false); } w.classList.add('dragging'); }
  handleDragMove(e){ const w = e.target; const x = (parseFloat(w.dataset.x)||0) + e.dx; const y = (parseFloat(w.dataset.y)||0) + e.dy; w.style.transform = `translate(${x}px, ${y}px)`; w.dataset.x = x; w.dataset.y = y; }
  handleDragEnd(e){ const w = e.target; w.classList.remove('dragging'); if (!w.classList.contains('locked')) { w.dataset.spawnX = w.dataset.x; w.dataset.spawnY = w.dataset.y; } if (this.gameState==='ASSEMBLING') this.checkGameState(); }
  handleDropZoneEnter(e){ const s = e.target; if (s.dataset.occupied === 'false') s.classList.add('can-accept'); }
  handleDropZoneLeave(e){ e.target.classList.remove('can-accept'); }
  handleDrop(e){ const w = e.relatedTarget; const s = e.target; if (s.dataset.occupied === 'false') { this.lockWagonToSlot(w,s); } s.classList.remove('can-accept'); if (this.gameState==='ASSEMBLING') this.checkGameState(); }
  unlockWagon(w, rts=false){ w.classList.remove('locked'); const sid = w.dataset.currentSlot; if (sid !== 'none' && this.slots[sid]) this.slots[sid].dataset.occupied = 'false'; w.dataset.currentSlot = 'none'; if (rts) this.returnWagonToSpawn(w); }
  lockWagonToSlot(w, s){ const sr = s.getBoundingClientRect(); const br = this.board.getBoundingClientRect(); const x = sr.left - br.left; const y = sr.top - br.top; w.style.transform = `translate(${x}px, ${y}px)`; w.dataset.x = x; w.dataset.y = y; w.classList.add('locked'); const sid = s.dataset.slotId; s.dataset.occupied = w.id; w.dataset.currentSlot = sid; }
  returnWagonToSpawn(w){ const x = w.dataset.spawnX; const y = w.dataset.spawnY; w.style.transform = `translate(${x}px, ${y}px)`; w.dataset.x = x; w.dataset.y = y; }

  checkGameState(){
    if (!this.board || this.gameState!=='ASSEMBLING') return;
    const locked = Array.from(this.board.querySelectorAll('.wagon-card.locked'));
    if (!locked.length){ this.updateGameStatus('grey'); if (this.nextStageBtn) this.nextStageBtn.style.display='none'; return; }
    let allCorrect = true;
    for (const w of locked){ if (w.dataset.isCorrect !== 'true') { allCorrect = false; break; } }
    const allSlotsOccupied = this.slots.filter(Boolean).filter(s => s.dataset.occupied !== 'false').length >= this.TOTAL_SLOTS;
    if (allCorrect && allSlotsOccupied && locked.length >= this.TOTAL_SLOTS){
      var ordered = [];
      try {
        var self = this;
        this.slots.forEach(function(slot){
          if (!slot || !slot.dataset) return;
          var occ = slot.dataset.occupied;
          if (!occ || occ === 'false') return;
          var wagon = document.getElementById(occ) || (self.board ? self.board.querySelector('#' + occ) : null);
          if (wagon && wagon.dataset && wagon.dataset.isCorrect === 'true') ordered.push(wagon);
        });
      } catch(_){}
      if (ordered.length < this.TOTAL_SLOTS){
        ordered = locked.filter(function(w){ return w.dataset && w.dataset.isCorrect === 'true'; }).sort(function(a,b){
          var as = parseInt(a.dataset.currentSlot || '0', 10);
          var bs = parseInt(b.dataset.currentSlot || '0', 10);
          return as - bs;
        });
      }
      this.orderedCorrectWagons = ordered;
      this.updateGameStatus('green');
      this.wagons.forEach(w => { if (w.dataset.isCorrect !== 'true') w.classList.add('hidden'); });
      if (this.nextStageBtn) this.nextStageBtn.style.display = 'block';
      interact('.wagon-card').draggable(false);
    } else {
      this.updateGameStatus('red');
      if (this.nextStageBtn) this.nextStageBtn.style.display = 'none';
      interact('.wagon-card').draggable(true);
    }
  }

  // Етап 2: Питання
  goToQuestionsStage(){
    this.applyState('QUESTIONS');
    if (this.nextStageBtn) this.nextStageBtn.style.display = 'none';
    this.wagons.forEach(w => { if (w.dataset.isCorrect !== 'true') w.classList.add('hidden'); });
    if (window.Game1Questions) { Game1Questions.generate(this); Game1Questions.displayCurrent(this); }
  }

  // Етап 3: Пам'ять
  goToRecallStage(){
    this.applyState('RECALL');
    if (this.memoryCheckBtn) this.memoryCheckBtn.style.display = 'none';
    if (this.recallFeedback) this.recallFeedback.textContent = '';
    if (this.recallResetBtn) this.recallResetBtn.style.display = 'inline-block';
    if (this.recallRetryBtn) this.recallRetryBtn.style.display = 'none';
    if (this.recallSubmitBtn) this.recallSubmitBtn.style.display = 'block';
    if (window.Game1Recall) { Game1Recall.setupGrid(this); Game1Recall.trapFocus(this, true); }
    this.updateGameStatus('grey', 'Пригадай 6 вагонів:');
  }
  submitRecall(){ if (window.Game1Recall) Game1Recall.submit(this); }

  // UI helpers
  showError(m){ if (window.Game1UI) Game1UI.showError(this,m); else alert(m); }
  updateGameStatus(s,m){ if (window.Game1UI) Game1UI.updateGameStatus(this,s,m); }

  // Utils
  getRandomElements(a,n){ return (window.UtilsRandom ? UtilsRandom.getRandomElements(a,n) : this.shuffleArray([...(a||[])]).slice(0,n)); }
  shuffleArray(a){ for (let i=a.length-1; i>0; i--){ const j = Math.floor(Math.random()*(i+1)); [a[i],a[j]]=[a[j],a[i]]; } return a; }

  destroy(){
    try{ window.removeEventListener('resize', this._debouncedScatter || this.scatterWagons); }catch(_){ }
    try{ this.nextStageBtn && this.nextStageBtn.removeEventListener('click', this.goToQuestionsStage); }catch(_){ }
    try{ this.memoryCheckBtn && this.memoryCheckBtn.removeEventListener('click', this.goToRecallStage); }catch(_){ }
    try{ this.recallSubmitBtn && this.recallSubmitBtn.removeEventListener('click', this.submitRecall); }catch(_){ }
    try{ this.recallResetBtn && this.recallResetBtn.removeEventListener('click', this._onRecallReset); }catch(_){ }
    try{ this.recallRetryBtn && this.recallRetryBtn.removeEventListener('click', this._onRecallRetry); }catch(_){ }
    try{ if (this.answerOptions) this.answerOptions.querySelectorAll('.answer-btn').forEach(b => b.onclick = null); }catch(_){ }
    try{ if (window.Game1Recall) Game1Recall.trapFocus(this, false); }catch(_){ }
    try{ if (window.Game1DragDrop) Game1DragDrop.unset(); }catch(_){ }
  }
}


/* ---- END js/train_game.js ---- */

/* ---- BEGIN js/app.js ---- */
/*
  ФАЙЛ: js/app.js
  ПРИЗНАЧЕННЯ: Спільні дрібні інтеракції сайту (бургер‑меню тощо).
  Примітка: файл не залежить від інших скриптів і може підключатися всюди.
*/
(function(){
  document.addEventListener('DOMContentLoaded', function(){
    ...
    menuToggle.addEventListener('click', function(){
      ...
      var opened = mainNav.classList.contains('nav-active');
...
/* ---- BEGIN js/game_bootstrap.js ---- */
/*
  ФАЙЛ: js/game_bootstrap.js
  ПРИЗНАЧЕННЯ: Запускає гру «Потяг» лише на сторінці гри.
  Логіка: шукає у DOM елементи гри; якщо є й задані параметри type/level — стартує Game1_Logic.
*/
(function(){
  document.addEventListener('DOMContentLoaded', function(){
    try{
      var isGame1 = document.getElementById('game-board') || document.querySelector('.game-container .train-area');
      if (!isGame1) return;
      var params = new URLSearchParams(window.location.search || '');
      if (!params.get('type') || !params.get('level')){
        // Параметри не задані — не стартуємо, сторінка залишається статичною
        return;
      }
      if (typeof Game1_Logic === 'function') new Game1_Logic(params);
    }catch(e){ console.error(e); }
  });
})();
/* ---- END js/game_bootstrap.js ---- */

/* ---- BEGIN js/index.js ---- */
/*
  ФАЙЛ: js/index.js
  ПРИЗНАЧЕННЯ: Логіка головної сторінки (index.html):
    - рендер карток/опцій для «Гра Потяг»;
    - обчислення доступних літер;
    - перемикання вигляду між розділами (вправи / гра / про сайт).
  Залежить від: window.G1.*, window.GAME_DATA (із js/data.js).
*/
(function(){
  var titleEl = document.getElementById('game-title');
  var subtitleEl = document.getElementById('game-subtitle');
  var optionsWrapper = document.getElementById('game-options-wrapper');
  ...
  var navLinks = document.querySelectorAll('[data-view-target]');

  // Дані гри 1: беремо з GAME_DATA або використовуємо запасний варіант
  var GAME = (window.GAME_DATA && window.GAME_DATA['1']) || {
    title: 'Гра Потяг',
    subtitle: 'Збирай потяг, тренуй вимову та пам’ять',
    optionsGroups: [
      { text: 'Овочі', targetPage: 'index.html' },
      { text: 'Фрукти', targetPage: 'index.html' },
      { text: 'Одяг', targetPage: 'index.html' },
  ...
  function resolveSectionView(view){
  ...
  function updateNav(activeView){
    ...
    Array.prototype.forEach.call(navLinks, function(link){
      ...
      var isActive = target === activeView;
  ...
  function applyView(view){
    var sectionView = resolveSectionView(view);
    Object.keys(viewSections).forEach(function(key){
      var section = viewSections[key];
      if (!section) return;
      if (key === sectionView) section.classList.remove('is-hidden');
      else section.classList.add('is-hidden');
    });
    var navView = (sectionView === 'game') ? 'home' : sectionView;
    updateNav(navView);
    try { document.body.setAttribute('data-current-view', sectionView); } catch(_){ }
  }

  function normalizeTypeTag(text){
    var raw = (text || '').trim().toLowerCase();
    var localMap = {
      'овочі':'овочі','фрукти':'фрукти','одяг':'одяг','тварини':'тварина','тварина':'тварина','птахи':'птах','птах':'птах','посуд':'посуд',
      'на початку слова':'на початку','в середині слова':'в середині','в кінці слова':'в кінці'
    };
    var map = (window.G1 && window.G1.TYPE_TAG_MAP) || localMap;
    return (raw in map) ? map[raw] : raw.replace(' слова','');
  }

  function buildGameHref(target, groupText, letter){
    var base = target || 'index.html';
    var separator = base.indexOf('?') >= 0 ? '&' : '?';
    return base + separator + 'view=game&type=' + encodeURIComponent(groupText) + '&level=' + encodeURIComponent(letter);
  }

  function createOptionRow(group, lettersOk){
    if (!Array.isArray(lettersOk) || lettersOk.length === 0) return null;
    ...
    wrap.className = 'game-options-buttons';

    lettersOk.forEach(function(letter){
      var a = document.createElement('a');
      a.className = 'letter-btn';
      a.textContent = letter.toUpperCase();
      a.href = buildGameHref(group.targetPage || 'index.html', group.text, letter);
      a.setAttribute('aria-label', group.text + ': літера ' + letter.toUpperCase());
      wrap.appendChild(a);
    });
  ...
  async function computeAvailableLettersFor(typeText){
    var tag = normalizeTypeTag(typeText);
    var reqCorrect = 6, reqIncorrect = 4;
    var letters = (window.G1 && Array.isArray(G1.LETTERS) && G1.LETTERS) || [];
    if (window.G1 && typeof G1.ensureLetterLoaded === 'function'){
      if (!window.G1_LETTERS_ENSURED){
        await Promise.allSettled(letters.map(function(l){ return G1.ensureLetterLoaded(l); }));
        window.G1_LETTERS_ENSURED = true;
      }
    }
    var ok = [];
    letters.forEach(function(l){
      try{
        var v = (window.G1 && typeof G1.validate==='function') ? G1.validate(l, tag, reqCorrect, reqIncorrect) : null;
        if (v && v.ok) ok.push(l);
      }catch(_){ }
    });
    return ok;
  ...
  async function renderGame1(){
    if (!titleEl || !subtitleEl || !optionsWrapper) return;
    titleEl.textContent = GAME.title; subtitleEl.textContent = GAME.subtitle;
    optionsWrapper.innerHTML = '<p style="color:#ccc">Готуємо список літер…</p>';

    var rows = [];
    for (var i=0; i<(GAME.optionsGroups||[]).length; i++){
      var group = GAME.optionsGroups[i];
      var ok = await computeAvailableLettersFor(group.text);
      var row = createOptionRow(group, ok);
      if (row) rows.push(row);
    }
    optionsWrapper.innerHTML = '';
    if (rows.length === 0) {
      optionsWrapper.innerHTML = '<p style="color:#ccc">Для обраної гри поки немає даних.</p>';
    } else {
      rows.forEach(function(r){ optionsWrapper.appendChild(r); });
    }
  }

  function setActiveCard(card){
    var all = document.querySelectorAll('.game-card');
    Array.prototype.forEach.call(all, function(c){ c.classList.toggle('active', c===card); });
  }
  ...
  async function handleCardClick(e){
    ...
    if (card.getAttribute('aria-disabled')==='true'){
      ...
      titleEl.textContent = titleNode ? titleNode.textContent : 'Гра';
      subtitleEl.textContent = 'Ця гра в розробці.';
      optionsWrapper.innerHTML = '';
      setActiveCard(card);
      return;
    }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    if (grid){
      ...
      grid.addEventListener('keydown', function(e){ if ((e.key==='Enter'||e.key===' ') && e.target.closest('.game-card')) { e.preventDefault(); handleCardClick(e); } });
    }
    try{
      var params = new URLSearchParams(window.location.search||'');
      var viewParam = params.get('view') || (params.get('type') && params.get('level') ? 'game' : 'home');
      applyView(viewParam);
      if (viewParam === 'game'){
        var cardGame = document.querySelector('.game-card[data-game-id="1"]');
        if (cardGame) cardGame.classList.add('active');
      }
      if (params.get('game') === '1'){
        var card = document.querySelector('.game-card[data-game-id="1"]');
